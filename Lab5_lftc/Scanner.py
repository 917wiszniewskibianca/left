...
class Scanner:
    ...
    def tokenizeLine(self, line_string):
        ...
        # we take the spaces out from the filtered elements
    ...
    def classifyToken(self, token):
        ...
        if string_match is not None:
            ...
            if char_match is not None:
                ...
                (isMatch, finalState) = self._FA.check_sequence(token)
                if isMatch and finalState == 'B': #identifier
                    return 1
                # int_match = re.match('^0$|^(\+|-)?[1-9][0-9]*$', token)
                elif isMatch and finalState in ('C', 'E'): # constant integer
                    return 4
        return 0
